...
import { updateProfileAvatar, updateProfileMetadata, deleteAgentAccount } from '@/app/actions/profile-actions'
...
export default function AgentSettingsPage() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const supabase = createClient()
      const result = await deleteAgentAccount(user?.id!)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      ...
      toast({ title: 'Success', description: 'Agent profile updated successfully.' })
    ...
    <div className="p-6 lg:p-10 max-w-[1400px] mx-auto animate-in fade-in duration-700 pb-32">
      ...
      <div className="space-y-0">
        <SettingSection 
          icon={User} 
          title="Agent Profile" 
          description="Manage your professional identity and personal details."
        >
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-neutral-600 font-bold">Contact Phone Number (Compulsory)</label>
              <Input 
                required
                type="tel"
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-neutral-600 font-bold">Professional Tagline / Role</label>
              <Input 
                value={profile?.tagline || ''} 
                onChange={e => setProfile({...profile, tagline: e.target.value})}
                placeholder="e.g. Senior Architectural Consultant"
                className="bg-neutral-900/40 border-neutral-800 text-white rounded-none h-12 focus-visible:ring-1 focus-visible:ring-neutral-700"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-neutral-600 font-bold">Professional Biography</label>
              <textarea 
                value={profile?.bio || ''} 
                onChange={e => setProfile({...profile, bio: e.target.value})}
                placeholder="Brief professional background..."
                className="w-full bg-neutral-900/40 border border-neutral-800 text-white rounded-none p-4 text-sm focus:outline-none focus:ring-1 focus:ring-neutral-700 min-h-[120px] transition-all"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] uppercase tracking-[0.2em] text-neutral-600 font-bold">External Portfolio / Website</label>
              <Input 
        ...
        <SettingSection 
          ...
          description="Upload a professional headshot to represent you across the platform."
        >
        ...
        <SettingSection 
          icon={Trash2} 
          title="Danger Zone" 
          description="Permanently delete your agent account and associated personal agency if applicable."
        >
          <div className="space-y-6">
            <div className="p-4 border border-red-950/50 bg-red-950/20 text-neutral-300 text-xs leading-relaxed space-y-2">
              ...
              <p>Deleting your account will permanently remove your profile, revoke access, and delete all associated data from the HousePadi Platform. If you are registered as an Independent Agent, your personal agency and Freelancer subscription will be deleted as well. This action is irreversible.</p>
            </div>
        ...
        <DialogContent className="bg-black/95 border border-neutral-800 text-white rounded-none sm:max-w-md p-8 backdrop-blur-xl">
          ...
          <div className="py-6 space-y-4">
            <p className="text-neutral-400 text-xs font-light leading-relaxed">
              Are you absolutely sure you want to delete your agent account? Please type <span className="font-mono text-red-400 font-bold">DELETE</span> below to confirm.
            </p>
