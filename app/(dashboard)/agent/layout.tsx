...
import { updateProfileMetadata } from '@/app/actions/profile-actions'
...
export default function AgentLayout({
  ...
  useEffect(() => {
    async function verifyRole() {
      ...
      if (error || !data || data.role !== 'AGENT') {
        router.push('/auth/login?error=unauthorized')
      } else {
        setProfile(data)
        setIsAuthorized(true)
        // If tagline is missing and not a personal agent, show onboarding
        if (!data.is_personal && !data.tagline) {
